...
sudo mysql --user="${DB_USERNAME}" --password="${DB_PASSWORD}" --execute="ALTER USER '${DB_USERNAME}'@'localhost' IDENTIFIED WITH mysql_native_password BY '${DB_PASSWORD}';"
if [ $? -eq 0 ]; then
    echo "User ${DB_USERNAME} altered successfully."
else
